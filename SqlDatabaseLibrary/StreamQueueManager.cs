...
namespace SqlDatabaseLibrary
{
    public static class StreamQueueManager
    {
        ...
        public static bool AdjustQueueSettings(int queueId, string updateText, queueAdjustmentTypes updateType)
        {
            var databaseConnection = SqlDatabaseConnection.Instance();
        ...
        public static List<QueueEntryModel> LoadStreamQueue(int queueId)
        {
            ...
            if (databaseConnection.TryDatabaseConnection())
            {
                ...
                string query = "SELECT * FROM QUEUE WHERE QUEUEID = @queueid";
                ...
                for (int i = 0; sqlReader.Read(); i++)
                {
                    ...
                    newQueueEntry.positionInQueue = sqlReader.GetInt32(1);
                    newQueueEntry.matchStatus = (QueueEntryModel.status)sqlReader.GetInt32(2);
                    newQueueEntry.roundInBracket = sqlReader.GetString(3);
